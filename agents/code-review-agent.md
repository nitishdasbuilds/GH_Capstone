...
Create `artifacts/code-review.md` using the structure below:
...
- Confirm completion: "Code review approved. The review document is ready at `artifacts/code-review.md`. Blocking issues (if any) must be resolved by the implementation agent before the next stage proceeds."

**If "Request Changes" is selected:**
- Ask: "What changes would you like to make to the review?"
- Wait for feedback
- Update `artifacts/code-review.md` based on feedback
...
1. **read_file**: To read `src/doc_sync.py`, `src/calculator.py`, `artifacts/requirements.md`, `artifacts/architecture.md`, `artifacts/impl-plan.md`
2. **get_errors**: To check for compile/lint issues as supporting evidence for Correctness/Clarity findings
3. **vscode_askQuestions**: To request human approval and gather change feedback
4. **create_file**: To create `artifacts/code-review.md`
5. **replace_string_in_file** / **multi_replace_string_in_file**: To update the review based on feedback
